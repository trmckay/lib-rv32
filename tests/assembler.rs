...
fn test_assemble_copious_commas() {
    ...
    assert_eq!(
        ...
        assemble_ir("addi,, t0,,, x6,, 0,,,", &mut empty_hash, 0).unwrap()
    );
}
...
fn test_assemble_i_type() {
    ...
    assert_eq!(
        ...
        assemble_ir("addi zero, x0, 17", &mut empty_hash, 0).unwrap()
    );
    assert_eq!(
        instructions::ADDI_X5_X6_NEG_12,
        assemble_ir("addi t0, t1, -12", &mut empty_hash, 0).unwrap()
    );
    assert_eq!(
        instructions::LW_X5_0_X5,
        assemble_ir("lw x5, 0(x5)", &mut empty_hash, 0).unwrap()
    )
}
...
fn test_assemble_u_type() {
    ...
    assert_eq!(
        ...
        assemble_ir("auipc x5, 4", &mut empty_hash, 0).unwrap()
    );
    assert_eq!(
        instructions::LUI_X5_4,
        assemble_ir("lui x5, 4", &mut empty_hash, 0).unwrap()
    );
}
...
fn test_assemble_b_type() {
    ...
    let actual = assemble_ir("beq x5, x5, 12", &mut empty_hash, 0).unwrap();
...
fn test_assemble_with_label() {
    ...
    assert_eq!(
        instructions::LUI_X5_4,
        assemble_ir("loop: lui x5, 4", &mut labels, 0).unwrap()
    );
